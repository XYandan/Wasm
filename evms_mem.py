...
ax.text(0.5, 59000, '×$10^4$', ha='center', va='bottom', fontsize=11)

# ax.set_xlabel('合约名', {'family': 'SimSun', 'weight': 'normal', 'size': 20})
ax.set_ylabel('Memory Occupancy (byte)', {'family': 'consolas', 'weight': 'normal', 'size': 15})
ax.set_xticks(x  +  (len(evm_memory) - 1) * width / 2.)
ax.set_xticklabels(contracts[:-1], rotation=30, fontsize=15, position = (0,0.01))
ax.legend()
...
legend = ax.legend(loc='upper right',bbox_to_anchor = (0.85,1) ,fontsize=11)
# plt.title('合约字节码长度比例')
plt.tight_layout()
plt.savefig('./fig/evms_mem.pdf', dpi=600, bbox_inches='tight')
plt.show()
