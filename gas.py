...
ax.set_ylabel('Gas(Wasm)/Gas(EVM)', {'family': 'consolas', 'weight': 'normal', 'size': 15})
# ax.set_yscale('log') # Y轴，10为底，科学计数
ax.text(0.25, 5.95, '$log_{2}$', ha='center', va='bottom', fontsize=12,fontstyle='normal')
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
# ax.set_xticks(x + width)
# ax.set_xticklabels(contracts[:-2], rotation=30, fontsize=20, verticalalignment='top')
# ax.xticks(contracts[:-2], rotation=30, fontsize=20, ha='right',position = (0,-1))
plt.xticks(x + width, contracts[:-2], rotation=30, fontsize=15, position = (0,-0.18))
ax.legend()

legend = ax.legend(loc='upper right',bbox_to_anchor = (0.85,0.9),fontsize=11)

plt.tight_layout()
plt.savefig('./fig/gas.pdf', dpi=600, bbox_inches='tight')
plt.show()
